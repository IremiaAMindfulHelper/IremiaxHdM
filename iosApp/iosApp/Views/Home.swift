        ...
        var body: some View {
            ScrollView {
                VStack(alignment: .leading, spacing: 25) {
                    ...
                    // FIX: Bindings an FilterBar übergeben
                    ...
                    if selectedFilter == "Alle" || selectedFilter == "Übungen" {
                        VStack(alignment: .leading, spacing: 15) {
                            HStack(alignment: .lastTextBaseline) {
                                ...
                                Spacer()
                                // FIX: Bindings an CategoryDetailView übergeben
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 10) {
                    ...
                    ForEach(categories, id: \.self) { category in
                        // FIX: Hier werden die Bindings für den Sound-Player an die Detailansicht gereicht
                    ...
                    if category == "Übungen" {
                        ...
                        VStack(spacing: 15) {
                            ForEach(WellnessData.sounds) { sound in
                                // FIX: Jetzt werden die echten Bindings statt Konstanten genutzt
    ...
    // MARK: - 5. UI KOMPONENTEN (HINWEIS: SoundRow nutzt jetzt @Binding)
        ...
        var body: some View {
            VStack(alignment: .leading, spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(exercise.beschreibung).font(.system(size: 12)).foregroundColor(.gray).lineLimit(2).multilineTextAlignment(.leading)
